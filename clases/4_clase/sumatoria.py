...
penRLn,    = plt.plot([],[],'b-',linewidth=2,alpha=0.8)
penILn,    = plt.plot([],[],'r-')
inversaAxe.grid(True)
inversaAxe.set_xlim(0,(N-1)/fs)
...
def init():
    global sumatoria,penRLn,inversaAxe
    inversaAxe.clear()
    inversaAxe.set_xlim(0,(N-1)/fs)
    inversaAxe.set_ylim(-1,1)
    penRLn,    = inversaAxe.plot([],[],'b-')
    sumatoria=np.zeros(N).astype(complex)
    return circleLn,circleLg,signalRLn,signalILn,massLn,promRLn,promILn,inversaLn,penRLn

def updateF(frecIter):
    global promData,fData,penData,sumatoria,penRLn,inversaAxe
    actualData   = promData[frecIter]*np.exp(1j*2*np.pi*((frecIter*(fs/N))-fs/2)*tData)
    sumatoria   += actualData
    inversaLn,   = plt.plot(tData,np.real(sumatoria),'y-',linewidth = 0.5,alpha = 0.8)
    penRLn.set_data(tData,np.real(sumatoria))
    return circleLn,circleLg,signalRLn,signalILn,massLn,promRLn,promILn,promZoneLn,inversaLn,penRLn

def updateT(frecIter):
    global circleData,signalData,promData,circleFrec,circleLg,signalsIter,sumatoria
