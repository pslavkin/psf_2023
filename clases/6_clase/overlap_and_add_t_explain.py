...
signalAxe.legend()
signalAxe.grid(True)
signalAxe.set_xlim(-1/fs,(NN+M-2)/fs)
...
segmentAxe.set_xlim(-1/fs,(N+M-2)/fs)
...
convAxe.legend()
convAxe.grid(True)
convAxe.set_xlim(-1/fs,(NN+M-2)/fs)
convAxe.set_ylim(np.min(convolveData)-0.2,np.max(convolveData)+0.2)
