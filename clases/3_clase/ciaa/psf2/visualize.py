...
def readSamples(adc,fft,N,trigger=False,th=0):
    ...
    i=0
    for t in range(N):
        sample = readFloat4File(streamFile)*1.65
        #part real plus imag
        fftBin = readFloat4File(streamFile) + 1j*readFloat4File(streamFile)
        state,nextI= {
...
def update(t):
    ...
    ciaaFftAbs=1/(1024*fs*N)*np.abs(ciaaFft)**2 #factor de conversion TODO: estudiar de donde sale
    ciaaFftLn.set_data ( (fs/N )*fs*time ,ciaaFftAbs)
    fftAxe.set_ylim ( 0 ,np.max(ciaaFftAbs)+0.006)
